    ...
    <template>
      <div class="filter-container">
        <common-form inline :formLable="formLable" :form="form">
          ...
          <!--<el-form-item>
            &lt;!&ndash; <el-button type="primary" @click="search">导出表格</el-button> &ndash;&gt;
      ...
      <el-table :data="tableList" stripe border fit empty-text="暂无数据" style="width: 100%;">
        ...
        <el-table-column prop="buildName" align="center" label="单元楼" width="100"></el-table-column>
        <el-table-column prop="floor" align="center" label="楼层" width="80"></el-table-column>
        <el-table-column
          prop="idCard"
          align="center"
          label="身份证号"
        ...
        <el-table-column prop="id" align="center" label="操作" class-name="small-padding fixed-width">
          <template slot-scope="{row}">
            ...
            <el-popover placement="top" width="160" v-if="row.status!=='通过'||row.status!=='拒绝'" v-model="row.visible">
              <p>确定要通过吗？</p>
      ...
      <el-drawer title="员工详情" :visible.sync="detailVis" size="40%" direction="rtl">
        <div class="dra-wrapper">
          <div class="content">
            ...
            <div class="detail-name">
              <div class="ls-cen">
                <span class="ls-cen-title">楼层：</span>
                <span class="ls-cen-con">{{detail.buildName}}</span>
              </div>
              <div class="ls-cen">
                <span class="ls-cen-title">楼层：</span>
                <span class="ls-cen-con">{{detail.floor}}</span>
              </div>
              <div class="ls-cen">
          ...
          <div class="footer">
            ...
            <el-popover placement="top" v-if="row.status!=='通过'||row.status!=='拒绝'" width="160" v-model="detail.visible">
              <p>确定要通过吗？</p>
              <div style="text-align: right; margin: 0">
...
export default {
  ...
  methods: {
    ...
    async getApartmentSelect() {
      let res = await getApartmentSelect();
      let resData = res.data || {};
      if (resData.code === 200) {
        let data = resData.data || {};
        // 列表
        ...
        this.$message({
          type: "error",
          message: "单元楼状态选择项: " + (resData.msg || "请求失败")
        });
      }
    },
    // 获取企业二维码
